...
export function ManagersTab({
  ...
  newFee, setNewFee, newPhone, setNewPhone, showSuccess, onAdd, onToggleBlock, onRemove,
  editingManager, setEditingManager, editFee, setEditFee, editPhone, setEditPhone, onUpdateFee
}: {
  ...
  editFee: string; setEditFee: (v: string) => void;
  editPhone: string; setEditPhone: (v: string) => void;
  onUpdateFee: (id: string) => void;
}) {
  return (
    <div className="space-y-6">
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <input type="text" placeholder="WhatsApp (DDD + Nº)" value={newPhone} onChange={(e) => setNewPhone(e.target.value)} className="input-field" />
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-2">
                          ...
                          <input type="text" value={editPhone} onChange={(e) => setEditPhone(e.target.value)} placeholder="WhatsApp (DDD+nº)" className="w-36 px-2 py-1 border rounded text-center text-xs" />
                        ...
                        <button onClick={() => { setEditingManager(manager.id); setEditFee(manager.platformFee.toString()); setEditPhone(manager.phone || ''); }} className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-lg text-sm hover:bg-yellow-200">
                          Taxa: {manager.platformFee}% {manager.phone ? `• 📞 ${manager.phone}` : '• 📞 (sem tel)'}
                        </button>
                      )}
